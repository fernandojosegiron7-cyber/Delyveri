...
VERSIÓN 1.4 — PROXY VERCEL
- La PWA ya no depende de CORS para la conexión principal.
- Nuevo endpoint /api/proxy en Vercel.
- El navegador habla con Vercel y Vercel habla con WordPress.
- Probar conexión muestra el error real si WordPress no responde.
- Mantiene conexión directa como respaldo.
- No cambia ni guarda una ubicación falsa al probar conexión.

IMPORTANTE AL SUBIR A GITHUB
Sube también la carpeta:
api/
  proxy.js

La estructura raíz debe quedar:
index.html
app.js
styles.css
...
VERSIÓN 1.4.1 — FIX SAFARI
- Corrige "Can't find variable: proxyGet".
- proxyGet y proxyPost ahora se cargan al inicio de app.js.
- Compatible con Safari/iPhone.
- Mantiene proxy Vercel para evitar problemas de CORS.


VERSIÓN 2.0 Pro: conserva proxy Vercel, GPS y estados; agrega estado operativo local del repartidor.


VERSIÓN 3.0 — OPERACIÓN PRO
- Logo y nombre se cargan automáticamente desde WordPress.
- Chat repartidor-cliente.
- Confirmación de entrega con código de 4 dígitos.
- Mantiene GPS, estados, proxy Vercel y seguimiento.


USO INDIVIDUAL
- Esta PWA se conecta a un solo restaurante.
- El logo, nombre y color se toman desde WordPress.
- Para otro restaurante, crea otro proyecto Vercel o reutiliza el mismo paquete con otra URL de WordPress.
