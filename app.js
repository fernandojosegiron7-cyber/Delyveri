...
async function proxyGet(wp, action, token) {
  const u = new URL(PROXY_URL, window.location.origin);
  u.searchParams.set('wp', wp);
  ...
  try {
    ...
    throw new Error(`Proxy HTTP ${r.status}: respuesta inválida`);
  }
...
async function proxyPost(wp, payload) {
...
async function loadDeliveryInfo(){
  ...
  try{
    let data;
    try{
      data=await proxyGet(ajax,'fg_driver_order_info',token);
    }catch(proxyErr){
      // Fallback directo por compatibilidad.
    ...
    if(!data.success){
      try{
        data=await proxyGet(ajax,'fg_get_driver_delivery_info',token);
      }catch(e){}
    }
    ...
    console.error('loadDeliveryInfo',e);
  }
}
...
els.testConfig.addEventListener('click',async()=>{
  ...
  try{
    const data=await proxyGet(ajax,'fg_driver_order_info',token);
    if(data.success){
      toast('Conexión correcta');
      loadDeliveryInfo();
    }else{
      toast('WordPress respondió: '+(data?.data?.message||'pedido no encontrado'));
    }
  }catch(e){
...
document.querySelectorAll('.stepper button').forEach(btn=>{
  btn.addEventListener('click',async()=>{
    ...
    try{
      const data=await proxyPost(ajax,{action:'fg_driver_update_status',token,status});
      if(data.success){
...
document.querySelectorAll('[data-driver-state]').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('[data-driver-state]').forEach(b=>b.classList.remove('active'));btn.classList.add('active');localStorage.setItem('fg_driver_state',btn.dataset.driverState);if(typeof toast==='function')toast('Estado: '+btn.dataset.driverState);}));
const ds=localStorage.getItem('fg_driver_state');if(ds)document.querySelectorAll('[data-driver-state]').forEach(b=>b.classList.toggle('active',b.dataset.driverState===ds));


async function fgLoadBranding(){
  try{
    const cfg=typeof getConfig==='function'?getConfig():null;
    const wp=cfg?.ajaxUrl||document.querySelector('#ajaxUrl')?.value||localStorage.getItem('fg_wp_ajax')||'';
    if(!wp)return;
    const d=await proxyGet(wp,'fg_public_branding','');
    const x=d?.data||d;
    const logo=x?.driver_logo_url||'';
    const name=x?.brand_name||'FG Delivery';
    const color=x?.brand_primary||'#111827';
    const img=document.getElementById('brandLogo'),nm=document.getElementById('brandName');
    if(img&&logo){img.src=logo;img.style.display='block'}
    if(nm)nm.textContent=name;
    document.documentElement.style.setProperty('--fg-brand-primary',color);
  }catch(e){}
}

async function fgDriverChatLoad(){
  try{
    const cfg=typeof getConfig==='function'?getConfig():null;
    const wp=cfg?.ajaxUrl||document.querySelector('#ajaxUrl')?.value||localStorage.getItem('fg_wp_ajax')||'';
    const token=cfg?.token||document.querySelector('#token')?.value||localStorage.getItem('fg_driver_token')||'';
    if(!wp||!token)return;
    const d=await proxyGet(wp,'fg_driver_chat',token);
    const msgs=d?.data?.messages||[];
    const box=document.getElementById('driverChat');if(!box)return;
    box.innerHTML=msgs.map(m=>`<div class="chat-bubble ${m.sender==='Repartidor'?'me':''}"><strong>${m.sender}</strong><br>${String(m.message||'').replace(/[<>&]/g,'')}</div>`).join('');
    box.scrollTop=box.scrollHeight;
  }catch(e){}
}
document.getElementById('driverChatSend')?.addEventListener('click',async()=>{
  const input=document.getElementById('driverChatInput'),message=(input?.value||'').trim();if(!message)return;
  try{
    const cfg=typeof getConfig==='function'?getConfig():null;
    const wp=cfg?.ajaxUrl||document.querySelector('#ajaxUrl')?.value||localStorage.getItem('fg_wp_ajax')||'';
    const token=cfg?.token||document.querySelector('#token')?.value||localStorage.getItem('fg_driver_token')||'';
    await proxyPost(wp,{action:'fg_driver_chat',token,message});input.value='';fgDriverChatLoad();
  }catch(e){}
});
document.getElementById('confirmDeliveryCode')?.addEventListener('click',async()=>{
  const code=(document.getElementById('deliveryCodeInput')?.value||'').trim(),msg=document.getElementById('deliveryCodeMsg');
  if(!/^\d{4}$/.test(code)){if(msg)msg.textContent='Escribe los 4 dígitos';return;}
  try{
    const cfg=typeof getConfig==='function'?getConfig():null;
    const wp=cfg?.ajaxUrl||document.querySelector('#ajaxUrl')?.value||localStorage.getItem('fg_wp_ajax')||'';
    const token=cfg?.token||document.querySelector('#token')?.value||localStorage.getItem('fg_driver_token')||'';
    const d=await proxyPost(wp,{action:'fg_confirm_delivery_code',token,code});
    if(msg)msg.textContent=d?.success?'✓ Entrega confirmada':(d?.data?.message||'Código incorrecto');
  }catch(e){if(msg)msg.textContent=e.message||'Error';}
});
fgLoadBranding();fgDriverChatLoad();setInterval(fgDriverChatLoad,5000);
